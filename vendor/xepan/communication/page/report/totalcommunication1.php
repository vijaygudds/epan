...
class page_report_totalcommunication1 extends \xepan\base\Page{

	public $title = "Communication Analysis Reports";
	...
	function page_index(){
		...
		$layout_type = $this->app->stickyGET('layout_type');
		...
		$type_field = $form->addField('xepan\base\DropDown','communication_type')->setValueList(['Call'=>'Call','Meeting'=>'Meeting','FollowupCall'=>'Followup Call','NotCommunicated'=>'Not Communicated'])->setEmptyText('All');
		...
		$layout_field = $form->addField('xepan\base\DropDown','layout_type');
		$layout_field->setEmptytext('All');
		$layout_field->setValueList(['PerDay'=>' Per Day']);
		...
		// if($layout_type){
			// $start_date = $this->api->stickyGET('from_date');
			// $end_date = $this->api->stickyGET('to_date');
			// if($start_date){
			// 	$date_diff_day = $this->app->my_date_diff($start_date."-1 day",$end_date);

			// 	for ($i=0; $i <= $date_diff_day['days'] ; $i++) {
			// 		echo "string". $i;
			// 	}	
			// 		// $holiday_date = date('Y-m-d', strtotime($model['month_from_date'] . ' +'.$i.' day'));
			// 	echo "<pre>";
			// 	print_r($date_diff_day);
			// 	echo "</pre>";
			// }
		...
		// }else{
			...
			if($to_date){
				...
				$model_field_array = ['total_communication','created_at'];//,'unique_lead','communication','total_email','total_comment','total_meeting','total_sms','total_telemarketing','total_call','dial_call','received_call','unique_leads_from','unique_leads_to','attended_others_meeting'];
			// sub type 1
			...
			// if($_GET['layout_type'])
				$emp_model->_dsql()->group('created_at');
			if($_GET['employee_id'])
		...
		$grid->addHook('formatRow',function($g){
			...
			$g->current_row_html['subtype_1'] = '<div class="row"><div class="col-md-5"> <div data-id="'.$g->model->id.'" sparkType="pie" sparkHeight="70px" class="sparkline subtype1"></div></div><div class="col-md-7"><small>'.$sub_type_1_label_str."</small></div></div>";
			...
			$g->current_row_html['subtype_3'] = '<div class="row"><div class="col-md-5"> <div data-id="'.$g->model->id.'" sparkType="pie" sparkHeight="70px" class="sparkline subtype3"></div></div><div class="col-md-7"><small>'.$sub_type_3_label_str."</small></div></div>";
	...
	function page_commdegging(){
		...
		$grid->template->tryDel('Pannel');
		$grid->setModel($comm_model,['title','description','created_at','from','to','to_contact_str','created_by','generated_by','sub_type','calling_status','sub_type_3','status','communication_for','communication_subfor']);
		$grid->addPaginator(100);
		$grid->addHook('formatRow',function($g){
				// $g->current_row_html['message'] = $g->model['message'];
