...
class page_report_employeeleadreport extends \xepan\base\Page{
	...
	function init(){
		...
		$emp_model = $this->add('xepan\marketing\Model_Lead',['from_date'=>$from_date,'to_date'=>$to_date]);
		...
		$grid->setModel($emp_model,['created_by','generated_by','name','address','city','pin_code','created_at','emails_str','contacts_str','last_communication','department_id']);
		...
		if($form->isSubmitted()){
			$grid->js()->reload(
							[
								...
								'leadcategory'=>$form['lead_category']?:0,
								'leadsubcategory'=>$form['lead_sub_category']?:0,
							]
				)->execute();
		}
