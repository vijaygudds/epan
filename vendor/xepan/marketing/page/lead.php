...
class page_lead extends \xepan\base\Page{
	...
	function page_index(){
		...
		// $lead_cat->js('change',$lead_subcat->js()->reload(null,null,[$this->app->url(null,['cut_object'=>$lead_subcat->name]),'leadcategory_id'=>$lead_cat->js()->val()]));
	...
	function formLayout($crud,$model, $other_fields){
		...
		$ge = $model->getElement('generated_by_id')->getModel();
		...
		$layout_array=[
					...
					'score~Score (Is Lead Positive or Negative)'=>'c12~2~or leave as it is for nutral',
					'score_buttons~'=>'c13~3',
					'landmark'=>'Address Info~a1~4',
					'mohla_falla'=>'c8~4',
					'village'=>'c9~4',
					'tehsil'=>'c10~4',
					'post_office_wardno'=>'c11~4',
		...
		if($crud->isEditing('edit')){
			...
			unset($layout_array['communication_for']);unset($layout_array['communication_for']);
			unset($layout_array['communication_sub_for']);unset($layout_array['communication_sub_for']);
		...
		if($crud->isEditing()){ // add or edit to save contact details
			$model->addHook('afterSave',function($m)use($crud, $other_fields){
				...
				$cats_from_field = is_array($crud->form['occupation'])?$crud->form['occupation']:explode(",",$crud->form['occupation']);
	...
	function processCommunicationSave($m,$form){
		...
		switch ($commtype) {
			// case 'Email':
			// 	$send_settings = $form->add('xepan\communication\Model_Communication_Email');
	...
	function page_import(){
		...
		if($_GET['download_sample_csv_file']){
			...
			$output = ['first_name','last_name','landmark','mohla_falla','village','tehsil','post_office_wardno','address','city','state','country','pin_code','organization','post','website','source','remark','personal_email_1','official_email_1','personal_contact_1','personal_contact_2','category','lead_cat_id','lead_cat_sub_id','assign_to_id','generated_by_id','lead_type'];
	...
	function page_grab(){
		...
		$category_field = $f->addField('Dropdown','occupation');
